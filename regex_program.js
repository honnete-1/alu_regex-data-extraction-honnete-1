...
/* URL pattern explanations:The pattern Only matches URLs with http or https protocol and
...
/* Phone number pattern explanations: It supports optional international prefix (e.g. +1) 
and accepts formats like (123) 456-7890 or 123-456-7890 */
  phoneNumbers: /(?:\+\d{1,3}[-.\s]?)?(?:\(\d{3}\)|\d{3})[-.\s]?\d{3}[-.\s]?\d{4}\b/g,
/* Credit card pattern explanations: It matches standard 16-digit cards with spaces or hyphens and 
also supports alternative grouping*/
  creditCards: /\b(?:\d{4}[-\s]?){3}\d{4}\b|\b\d{4}[-\s]?\d{6}[-\s]?\d{5}\b/g,
/*Time pattern explanations: It supports 24-hour format (e.g. 14:30), supports 12-hour format 
with AM/PM (e.g. 02:45 PM) and some invalid combinations are filtered later in validation logic*/
  time: /\b(?:[01]?\d|2[0-3]):[0-5]\d(?:\s?(?:AM|PM|am|pm))?\b/g,
/* HTML tag pattern explanations:It matches opening and self-closing HTML tags, supports attributes 
with quoted or unquoted values, and is used only for extraction, not for parsing or sanitizing HTML*/
  htmlTags: /<[a-zA-Z][a-zA-Z0-9]*(?:\s+[a-zA-Z][a-zA-Z0-9-]*(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+))?)*\s*\/?>/gi,
// Hashtag pattern: Must start with a letter and allows numbers and underscores after, and prevents matching broken hashtags like ##
  hashtags: /#[a-zA-Z][a-zA-Z0-9_]*\b/g,
/* Currency pattern: Matches dollar amounts with optional comma separators, Requires two decimal places 
